...
export default function scrapeIchingTable() {
  ...
  router
    ...
    .scrape($ => {
      ...
      let trigrams_raw = $(
        "tr:nth-of-type(3) td tr td:nth-of-type(2) pre",
        tbody
      ).get();
      let trigrams = [];
      trigrams_raw.map(v => {
      ...
      let resume = interp_raw
        .match(/[\s\S]*?THE JUDGMENT/)[0]
        .replace("THE JUDGMENT", "");
      let judgment = interp_raw
        .match(/THE JUDGMENT[\s\S]*?THE IMAGE/)[0]
        .replace("THE JUDGMENT", "")
        .replace("THE IMAGE", "");
      let image = interp_raw
        .match(/THE IMAGE[\s\S]*?THE LINES/)[0]
        .replace("THE IMAGE", "")
        .replace("THE LINES", "");

      let lines_raw = $("tr:nth-of-type(4) tr td pre p", tbody).text();
      let lines_clean = lines_raw
        .match(/THE LINES[\s\S]*?$/)[0]
        .replace("THE LINES", "");
