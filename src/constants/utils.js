import { isEqual } from 'lodash'

/* Utils functions */
export function getAsset( resource_path ) {
  return (__DEVELOPMENT__ ? `/public/${resource_path}` : `assets/${resource_path}`)
}

export function CSSVar( varName , rootCtx = document.body) {
  return rootCtx.style.getProperty(varName);
}

/**
 * create a function that will filter http responses whithin range min<max
 * and throw error otherwise. Useful in fetch promises
 */
export function filterHTTP(min, max) {
  return (response) => {
    if (response.status >= min && response.status < max) {
      return response
    } else {
      var error = new Error(response.statusText)
      error.response = response
      throw error
    }
  }
}

/** is(x).equal(p) */
export function is( V ) {
  return {
    in: (A) => (A.indexof(V) != -1),
    equal: (A) => (isEqual(A,V))
  }
}

/**
 * Generate a crypto UUID RFC4122 compliant
 * https://30secondsofcode.org/#uuidgeneratorbrowser
 */
export const UUIDGenerator = () =>
  ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, c =>
    (c ^ (crypto.getRandomValues(new Uint8Array(1))[0] & (15 >> (c / 4)))).toString(16)
  );